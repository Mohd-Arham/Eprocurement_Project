...
public class CircleDAOImpl implements CircleDAO<Circle>{
    ...
    @Override
    public String checkDuplicateValue(Circle circle,Integer deptId) {
        System.out.println("-----------------Method called to checkDuplicateValue Circle in Dao----------------------------");
        String message = "";
        List<Circle> circleList = this.fetchAllCirclebyDept_ID(deptId);
    ...
    @Override
    public List fetchAllCirclebyDept_ID(Integer id) {
         System.out.println("----------------Method call to fetchAllbyDept_ID subdivision in  SubDivisionDaoImpl----------------");
        List<Circle> circleList = new ArrayList<Circle>();
        Session session=null;
        try {
             session= sessionFactory.openSession();
            Query query = session.createQuery("FROM Circle where dept_Id=:id");
            query.setParameter("id", id);
            System.out.println("dept id ~~~"+id);
            circleList = query.list();
        }catch(Exception ex){
         System.out.println("Class: CircleDaoImpl \n Method:fetchAlCirclelbyDept_ID");
         ex.printStackTrace();
     }finally{
         if(session != null){
             session.close();
        ...
        System.out.println("----------------Method end to fetchAllCirclebyDept_ID  in  CircleDaoImpl----------------");
         return circleList ; 
    }
    }
